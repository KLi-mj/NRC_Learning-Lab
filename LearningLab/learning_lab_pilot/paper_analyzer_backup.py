...
키워드 (쉼표로 구분):"""
    ...
    def visualize_clusters(self):
        ...
        plt.title('논문 클러스터링 결과 (PCA 2D 시각화)', fontsize=16, pad=20)
        plt.xlabel(f'PC1 (설명 분산: {pca.explained_variance_ratio_[0]:.1%})', fontsize=12)
        plt.ylabel(f'PC2 (설명 분산: {pca.explained_variance_ratio_[1]:.1%})', fontsize=12)
        
        # 클러스터 중심점 표시
        cluster_centers_2d = []
        for cluster_id in range(len(np.unique(self.clusters))):
            cluster_points = embeddings_2d[self.clusters == cluster_id]
        ...
        # 파일 저장
        plt.savefig(Config.OUTPUT_CONFIG['charts_filename'], dpi=300, bbox_inches='tight')
        print(f"📊 시각화 저장: {Config.OUTPUT_CONFIG['charts_filename']}")
        plt.show()
